...
class WallpaperXApp extends StatelessWidget {
  ...
  const WallpaperXApp({super.key, required this.showOnboarding});

  @override
  Widget build(BuildContext context) {
    final themeProvider = Provider.of<LegacyThemeProvider>(context);
    return MaterialApp(
      title: 'WallpaperX',
...
class _HomeScreenState extends State<HomeScreen>
  ...
  bool get isDarkMode =>
      Provider.of<LegacyThemeProvider>(context, listen: false).isDarkMode;
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            onTap: () {
              ...
              if (homeScreen != null) {
                homeScreen._onCategoryTap(category['name']);
              }
            },
            child: Container(
              decoration: BoxDecoration(
...
class _SearchResultsScreenState extends State<SearchResultsScreen> {
  ...
  Future<void> performSearch({bool reset = true}) async {
    ...
    try {
      ...
      final response = await _dio.get(
        'https://api.pexels.com/v1/search?query=${widget.searchQuery}&per_page=8&page=$randomPage',
        options: Options(headers: {'Authorization': apiKey}),
      );
